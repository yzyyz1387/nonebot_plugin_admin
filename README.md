...
```
或者在`pyproject.toml`中的`plugins = `追加：
```toml
...
- `事件通知`
- `防撤回`
- `欢迎词+内容`：设置本群入群欢迎词；已有欢迎词时需回复 `1` 或 `确认` 覆盖。
- `欢迎词-`：删除本群欢迎词；需二次确认。

欢迎词仅由群主、管理员或超级用户设置，内容保存在 ORM 数据库的 `admin_group_welcome_word` 表中。入群欢迎消息仍需开启“事件通知”功能开关。
