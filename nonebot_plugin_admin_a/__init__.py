...
@admin_init.handle()
async def init(bot: Bot, event: MessageEvent, state: T_State):
    """
    初始化配置文件
    :return:
    """
...
# 查看当前群分管
gad = on_command('查看分管', aliases={"/gad", "/分群管理"}, priority=1, block=True,
...
# 开启superuser接收处理结果
su_gad = g_admin = on_command('群管接收', priority=1, block=True, permission=SUPERUSER)
...
async def _(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if len(msg) == 3:
        gid = msg[1]
        anwser = msg[2]
        sp_write = await approve.write(gid, anwser)
        if gid.isdigit():
            if sp_write:

                await super_sp_add.send(f"群{gid}添加入群审批词条：{anwser}")
            else:
                await super_sp_add.send(f'{anwser} 已存在于群{gid}的词条中')
        else:
